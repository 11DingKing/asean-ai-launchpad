...
func (w *Worker) runOne(ctx context.Context) {
	...
	err = w.handle(processingCtx, job)
	if err == nil {
		if completeErr := w.Store.CompleteJob(processingCtx, job.ID, w.Owner, w.Clock.Now()); completeErr != nil {
			w.Log.Error("complete durable job", "job_id", job.ID, "error", completeErr)
		}
		return
	}
	if retryErr := w.Store.RetryJob(processingCtx, job, err.Error(), w.Clock.Now()); retryErr != nil {
