...
type Scenarios interface {
	...
	ListScenarios(context.Context, string, domain.ScenarioStatus, domain.Page) ([]domain.Scenario, int, error)
...
type Metering interface {
	RecordUsage(context.Context, domain.UsageRecord) error
	RecordSettlement(context.Context, domain.SettlementEntry, domain.AuditEvent) error
