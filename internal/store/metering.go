...
func (s *Store) RecordUsage(ctx context.Context, usage domain.UsageRecord) error {
	_, err := s.db.ExecContext(ctx, `INSERT INTO usage_records(id,deployment_id,partner_id,period_start,period_end,unit_seconds,created_at) VALUES(?,?,?,?,?,?,?)`, usage.ID, usage.DeploymentID, usage.PartnerID, timestamp(usage.PeriodStart), timestamp(usage.PeriodEnd), usage.UnitSeconds, timestamp(usage.CreatedAt))
	return classify(err, "record usage")
}

func (s *Store) RecordSettlement(ctx context.Context, settlement domain.SettlementEntry, audit domain.AuditEvent) error {
	return s.transaction(ctx, func(tx *sql.Tx) error {
		if _, err := tx.ExecContext(ctx, `INSERT INTO settlement_entries(id,usage_id,partner_id,amount_micros,currency,created_at) VALUES(?,?,?,?,?,?)`, settlement.ID, settlement.UsageID, settlement.PartnerID, settlement.AmountMicros, settlement.Currency, timestamp(settlement.CreatedAt)); err != nil {
			return classify(err, "record settlement")
		}
