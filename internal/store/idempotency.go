...
func (s *Store) ReclaimIdempotency(ctx context.Context, item domain.IdempotencyRecord) (domain.IdempotencyRecord, bool, error) {
	result, err := s.db.ExecContext(ctx, `DELETE FROM idempotency_records WHERE scope=? AND response_status=0`, item.Scope)
	if err != nil {
		return domain.IdempotencyRecord{}, false, classify(err, "reclaim in-flight idempotency key")
	}
	changed, err := result.RowsAffected()
	if err != nil {
		return domain.IdempotencyRecord{}, false, err
