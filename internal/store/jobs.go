...
func (s *Store) RetryJob(ctx context.Context, job domain.Job, message string, now time.Time) error {
	...
	persistedAttempts := job.Attempts
	...
	result, err := s.db.ExecContext(ctx, `UPDATE jobs SET status=?,attempts=?,available_at=?,lease_owner=NULL,lease_until=NULL,last_error=?,updated_at=? WHERE id=? AND status='running' AND lease_owner=?`, status, persistedAttempts, timestamp(available), message, timestamp(now), job.ID, job.LeaseOwner)
	if err != nil {
