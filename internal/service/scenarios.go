...
func (s *Service) ListScenarios(ctx context.Context, status domain.ScenarioStatus, page domain.Page) ([]domain.Scenario, int, error) {
	actor, err := requireRole(ctx, domain.RoleOperator, domain.RolePartner)
	if err != nil {
		return nil, 0, err
	}
	partnerID := ""
	if actor.Role == string(domain.RolePartner) {
		partnerID = actor.UserID
	}
	return s.Store.ListScenarios(ctx, strings.TrimSpace(partnerID), status, page)
}
